...
class Holidays:
    ...
    def get_holidays(self, country: str):
        ...
        response = requests.get(
            f"https://calendarific.com/api/v2/holidays?api_key={TOKEN}&country={country}&year={YEAR}"
        )
    ...
    def get_data(self):
        ...
        if os.path.isfile(self.holidays_path):
            with open(self.holidays_path, "r") as f:
                ...
                harvested_data = datetime.strptime(
                    cached_data.get("data_harvest_date"), "%Y-%m-%d %H:%M:%S.%f"
                )
                if abs((harvested_data - current_day).days) < 2:
                    logger.info(
                        "Using cached data from " + cached_data.get("data_harvest_date")
                    )
                    return cached_data
    ...
    def check_close_days(self):
        ...
        for name, dates in data.items():
            ...
            for date in dates:
                holiday_date = datetime.strptime(date["date"], "%d-%m-%Y").replace(
                    ...
                    close_days.append(
                        {
                            ...
                            "days_until": "Today!"
                            if holiday_date.day == current_day.day
                            else days_until + 1,
                        }
                    )
